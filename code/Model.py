...
class CountMeanAndCovOfFeature(nn.Module):
    ...
    def getSample(self, input):
        '''
        input: (batchSize, 384)，一般是Source Domain的Feature或者Target Domain的Feature
        这个函数的作用： 如果input是来自Source Domain的Feature，那么这个getSample函数就是TargetMean的getSample函数，
        TargetMean里边保存着一个通过k-means计算而来的shape是（7， 384）的七个类别平均特征running_mean，那么这个getSample函数
        就是用来计算input里边每一个384对应距离running_mean里边哪一个类别最相似，然后返回的就是最相似的running_mean的数组。
        '''
        if self.training:
...
class CountMeanOfFeatureInCluster(nn.Module):
    ...
    def forward(self, input): # input's shape is (batchsize, 384)
    ...
    def getSample(self, input): # input's shape is (batchsize, 384)
